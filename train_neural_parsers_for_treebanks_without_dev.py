...
if len(sys.argv)<7:
	print 'parser_path treebank_folder_path embedding_path output_folder t languages'
	sys.exit(0)
...
for i, language in enumerate(languages):
	...
	command = 'python -u ' + parser_path+ '/src/parser.py  --outdir '+ output_folder+language+' --train '\
	 + treebank_path+ '/'+language + ' --extrn ' + embedding_path + language + '.gz --t '+ str(t) +' > '+ output_folder+language+'/nohups.out '
