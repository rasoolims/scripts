...
for i in range(len(orig_trees)):
	...
	if score_orig < score_reorder:
		better += 1
		final_writer.write(reorder_trees[i].conll_str()+'\n\n')
	...
	orig_str = ' '.join([orig_trees[i].words[j] for j in range(0,len(orig_trees[i].words))])
	reorder_str =' '.join([reorder_trees[i].words[j] for j in range(0,len(reorder_trees[i].words))])
