...
class Reorderer:
	...
	def __init__(self, src_lm_path, a1, a2):
		print 'reading lm'
		self.src_lm = LanguageModel(src_lm_path)
		print 'reading lm done!'
		self.a1 = a1
		self.a2 = a2
	...
	def score(self, tree):
		...
		words = ['<s>','<s>','<s>','<s>']+tree.words +['</s>']

		l2 = self.src_lm.score_arr(words)

		sc = (self.a1*l2-self.a2*distance)
...
if __name__ =='__main__':
	...
	reorderer = Reorderer(src_lm_path,a1,a2)
	...
	cnt = 0
	for tree in trees:
		writer.write(reorderer.reorder_tree(tree, 1).conll_str()+'\n\n')
		cnt+=1
		if cnt%100 ==0:
