...
if len(sys.argv)<5:
	print 'python get_align_from_diag_file.py [src_file] [dst_file] [diag_file] [output_file]'
	sys.exit(0)
...
for word_pair in joint_word_count.keys():
	...
	writer.write(word_pair+'\t'+str(src_word_count[src])+'\t'+str(dst_word_count[dst])+'\t'+str(pmi)+'\t'+str(pmi2)+'\t'+str(joint_word_count[word_pair])+'\t'+str(all_count)+'\n')
	cnt+=1
	if cnt%10000==0:
