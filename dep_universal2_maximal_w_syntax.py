...
phrase_dict = {'cc': 'other', 'det': 'mod', 'orphan': 'other', 'reparandum': 'other', 'ccomp': 'mod', 'csubj': 'subj', 'conj': 'other', 'amod': 'mod', 'vocative': 'mod', 'mark': 'mod', 'case': 'mod', 'discourse': 'other', 'advcl': 'mod', 'dislocated': 'other', 'aux': 'aux', 'parataxis': 'mod', 'flat': 'mod', 'nsubj': 'subj', 'nummod': 'mod', 'advmod': 'mod', 'punct': 'other', 'compound': 'mod', 'xcomp': 'mod', 'cop': 'aux', 'obl': 'mod', 'obj': 'obj', 'dep': 'other', 'appos': 'mod', 'list': 'other','clf': 'other', 'nmod': 'mod', 'acl': 'mod', 'iobj': 'obj', 'expl': 'mod', 'root': 'root','fixed':'mod','goeswith':'mod'}
...
def obtain_non_recursive(tree, i, phrases, separation_sets):
	...
	if i!=0: 
		...
		phrases.append([span[0],span[-1],tree.tags[i-1], phrase_dict[tree.labels[i-1]]])

	for d in separation_sets[i]:
		obtain_non_recursive(tree, d, phrases, separation_sets)


def create_shallow_tree(tree, put_to_tag,use_ftag,head_dep_set):
	phrases = []
	...
	starts = {s:i for i,[s,e,p,hd] in enumerate(phrases)}
	...
	for starts, index in sorted(starts.iteritems(), key=lambda (k,v): (v,k)):
		...
		tree.ftags[phrase[0]-1] = 'B-'+pl+'_'+phrase[3] if pl!='O' else 'O'
		
		if pl!='O': head_dep_set[phrase[3]]+=1

		if phrase[1]>phrase[0]:
			for i in range(phrase[0]+1,phrase[1]+1):
				if use_ftag:
					tree.tags[i-1] = tree.ftags[i-1]
				tree.ftags[i-1] = 'I-'+pl+'_'+phrase[3] if  pl!='O' else 'O'
...
print 'writing trees'
full = 0
head_dep_set = defaultdict(int)
for tree in t:
	writer.write(create_shallow_tree(tree, put_to_tag, use_ftag,head_dep_set))
	is_full = True
