...
if len(sys.argv)<5:
	print 'python create_projection_instances.py [src_mst_file] [dst_mst_file] [align_intersection_file] [output_file_name] [is_labeled]'
	sys.exit(0)
	...
	for mod in range(0,len(src_tree[0])):
		...
		if alignment.has_key(src_mod) and alignment.has_key(src_head):
			...
			try:
				...
				instance_lst.append('dmpdd:'+dst_mod_pos+'|'+str(dst_distance)+'|'+dst_direction)
				instance_lst.append('dspdd:'+dst_head_pos+'|'+str(dst_distance)+'|'+dst_direction)
				...
				len_diff=abs(dst_len-len(src_tree[1]))
