...
if len(sys.argv)<9:
	print 'python project_alignment_with_meta_classifier_probs.py [learning_model_file] [src_mst_file] [dst_mst_file] [align_intersection_file] [src_pos_map] [dst_pos_map] [output_file_name] [is_labeled]'
	sys.exit(0)
	...
	if True:
		for i in range(0,len(dst_tree[0])):
			...
			if not fin_feats.has_key(i):
				...
				orig=dst_tree[5][i]
				neg_score=classfier.score(fin_feats[i],'0',True)
				pos_score=classfier.score(fin_feats[i],'1',True)
				pos_prob=math.exp(pos_score)/(math.exp(pos_score)+math.exp(neg_score))
				if str(no_restriction_heads[i])==orig:
					correct_deps+=1
				else:
					wrong_deps+=1
				dst_tree[3][i]=str(no_restriction_heads[i])
				dst_tree[2][i]=no_restriction_labels[i]+':'+str(pos_prob)
